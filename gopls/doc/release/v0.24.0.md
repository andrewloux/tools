...
In headless MCP mode, file watching now skips roots that do not contain a
`go.mod` or `go.work` file. Clients that do not support MCP roots use the
initial working directory, subject to the same check. This allows newly
created files to be discovered without watching unrelated parent directories.
<!-- golang/go#76291, golang/go#74598 -->
